...
const incomeMap: Record<string, string> = {
  ...
  '1m_2m':  '١م — ٢م ل.س',
  '2m_4m':  '٢م — ٤م ل.س',
  over_4m:  'أكثر من ٤م ل.س',
};

// ١. تصدير الطلبات
export const exportRequests = async (requests: any[], filename = 'تقرير-الطلبات') => {
    const headers = [
    'رقم المرجع', 'الاسم الكامل', 'الهاتف', 'الرقم الوطني', 'البريد', 'العمر', 'الجنس',
  ...
  const widths = [14, 20, 14, 16, 22, 6, 8, 12, 18, 12, 12, 18, 10, 14, 10, 14, 16, 14, 14];
  await generateFile([{
    name: 'الطلبات',
    setup: (ws) => {
      ...
      requests.forEach((r, i) => {
        ...
        addDataRow(ws, [
          r.ref_number, r.full_name, r.phone,r.national_id || '—', r.email || '—', r.age,
          genderMap[r.gender] || r.gender, r.region, r.address || '—',
          r.family_members, r.children_count || 0, incomeMap[r.income_range] || '—',
          ...
          15: { color: statusColor, bold: true },
        });
      });
